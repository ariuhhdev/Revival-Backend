...
export interface ResponseBody {
  ...
  intent: string | Intents;
  validationFailures?: Record<string, object>;
}

type Intents = "prod-live" | "prod" | "live";
...
export class ApiError {
  ...
  withMessageVar(variable: string[]): this {
    this.response.messageVars = variable;
    return this;
  }
  ...
  apply(c: Context): ResponseBody {
    ...
    c.status(this.statusCode as StatusCode);
  ...
  throwHttpException(): never {
    ...
    throw new HTTPException(this.statusCode as StatusCode, { res: errorResponse });
  }
