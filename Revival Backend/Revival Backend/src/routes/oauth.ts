...
export default function () {
  app.post("/account/api/oauth/token", async (c) => {
    ...
    if (body.grant_type === "exchange_code" && body.exchange_code) {
      ...
      const codeEntry = codes.find((e: any) => e.exchange_code === exchangeCode);
      if (codeEntry) {
        const accountId = codeEntry.accountId as string;
        logAuth("exchange_code login", { accountId });
